...
const Cart = () => {
  ...
  const totalAmount = cart.reduce(
    (total, product) => total + product.price * product.quantity,  // Correct calculation
    0
  );

  const handleDecrement = (productId: string) => {
    decrementQuantity(productId);  // Pass product.id directly (as a string)
  ...
  const checkout = async () => {
    await fetch("http://localhost:3000/api/checkout", {
      ...
      if (response.url) {
        ...
        // You could redirect to response.url for example
        // window.location.href = response.url;
      }
    })
  }
      ...
      {cart.length === 0 ? (
        ...
        <ul>
          {cart.map((product, index) => {
            // If `product.id` is missing, fallback to `index` as key (not recommended long-term)
